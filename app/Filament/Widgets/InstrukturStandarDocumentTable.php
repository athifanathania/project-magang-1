...
class InstrukturStandarDocumentTable extends BaseWidget
{
    protected static ?int $sort = 7; 
    protected int | string | array $columnSpan = 'full';
    protected static ?string $heading = 'Monitoring Dokumen Instruksi Standar';

    public function table(Table $table): Table
    {
        return $table
            ->query(
                // UBAH JADI OLDEST (Urut dari awal input)
                ImmInstruksiStandar::query()->oldest()
            )
            ->columns([
                    ...
                    ->getStateUsing(function ($record) {
                        return ImmLampiran::query()
                            ...
                            ->whereIn('documentable_type', [
                                'App\Models\ImmInstruksiStandar',
                                'ImmInstruksiStandar',
                                'instruksi_standar',
                            ])
                            ->whereNull('parent_id') 
